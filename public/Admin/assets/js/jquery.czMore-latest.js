...
(function ($, undefined) {
    ...
    $.fn.czMore = function (options) {
        ...
        return this.each(function () {
            let obj = $(this);
            let i = recordsetCount();
            let divPlus = '<div id="btnPlus" class="btnPlus"/>';
            let count = '<input id="' + this.id + options.countFieldPrefix + '" name="' + this.id + options.countFieldPrefix + '" type="hidden" value="0" size="5" />';

            obj.before(count);
            let recordset = obj.children("#first");
            obj.after(divPlus);
            let set = recordset.children(".recordset").children().first();
            let btnPlus = obj.siblings("#btnPlus");

            if(!options.styleOverride) {
                btnPlus.addClass('btn btn-success w-100')
                btnPlus.append($('<i>' , {
                    class : 'mdi mdi-plus text-white',
                }))
            }
            ...
            function loadMinus(recordset) {
                ...
                if(!options.styleOverride) {
                    btnMinus.addClass('btn btn-danger float-end')
                    btnMinus.append($('<i>' , {
                        class : 'mdi mdi-close fs-3',
                    }))
